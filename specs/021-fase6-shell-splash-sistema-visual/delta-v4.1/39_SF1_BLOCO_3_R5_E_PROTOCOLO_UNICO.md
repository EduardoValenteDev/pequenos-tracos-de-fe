...
## `ERRATA-07` — catorze defeitos deste artefato, apurados na retomada (aditiva; nada acima foi reescrito)

> **Origem.** Missão de retomada determinística após perda de sessão, conduzida inteiramente do lado
> do *host*, em modo leitura. Nenhum arquivo acima foi alterado; nenhuma conclusão histórica foi
> apagada. Cada ponto abaixo foi verificado por leitura direta do arquivo citado **nesta sessão**.
> **Nenhum ponto desta errata concede portão, declara `PASS` ou responde arbitragem do fundador.**

| # | Onde | O que o texto diz | Correção apurada |
|---|---|---|---|
| `E7-1` | `:66`, `:976`, `:998`, `:1057` | *"**Cinco** arbitragens bloqueiam o Bloco 3"* · *"5 arbitragens bloqueadoras"* · *"5 grupos / 6 linhas"* | As arbitragens bloqueadoras **nomeadas** em §12.1 são **seis**: `ARB-JS-BINARIO`, `ARB-28-ESCOPO`, `ARB-TAR-REDE`, `ARB-INJ-CAMINHO`, `ARB-28-7-TIRO-UNICO`, `ARB-OBRA-NOVA`. *"Cinco"* só é verdadeiro como contagem de **grupos**, e o agrupamento que produz cinco **pressupõe** que o fundador responda `ARB-28-7-TIRO-UNICO` e `ARB-OBRA-NOVA` **juntas** (`:1000-1001`). Essa pressuposição **não é decisão lavrada**. A contagem correta de arbitragens é **6**; a de grupos, **5 sob uma hipótese ainda não confirmada**. Propaga-se a `37:1077`, `38:504` e `38:512` |
| `E7-2` | `:530-531` | *"Todo salvamento legítimo do produto fora de injeção. `GH2b`, `GH3` … Não há backup, e `deleteBlob` já apagou o anterior"* | **Mecanismo destrutivo nomeado errado, e falso para `GH3`.** (a) Em `GH2b` o `deleteBlob` **não é** a causa: `saveArt` nunca o chama (só `deleteArt`, `atelierStorage.js:156,:159`). A destruição é **sobrescrita no mesmo caminho determinístico** — `fileBlobStore.js:127` (*"Idempotente: regrava o mesmo caminho determinístico"*), `:138-140`. O instante de irreversibilidade é `atelierStorage.js:106`. (b) Para `GH3` a afirmação é **simplesmente falsa**: em *slot* vazio, `coloring60DrawingStorage.js:657` (`if (oldUri && oldUri !== newUri)`) **nunca executa**, logo `deleteBlob` **não roda**. `GH3` é **aditivo** no eixo dos píxeis. **A conclusão de risco de `GH2b` permanece; a causa escrita é que estava errada** |
| `E7-3` | `:660-661` | *"`E1` grava conclusão em área protegida … e **não tem desfazimento cirúrgico**: `coloring60ResetService.js:139` reseta o catálogo inteiro"* | **Contraditado por código vivo.** Existe desfazimento cirúrgico **alcançável pelo operador dentro do produto**: `ColoringScreen.js:1418` (`accessibilityLabel="Limpar desenho"`) → `handleC60ClearDrawing` (`:1203`) → `:1222-1223`, que chama `clearColoring60Done` e `clearColoring60SavedDrawing`. `coloring60ActivityService.js:167-170` remove `done` **e** `snap` por `multiRemove`. **A simetria é parcial**, e a assimetria é deliberada: `hasEverCompleted` (`ever`) **não** é removido — `ColoringScreen.js:1201-1202` (*"limpar uma folha não apaga a memória de quem já chegou lá — só o reset canônico faz isso"*). **Contabilidade exata do que `E1` escreve:** `markColoring60ActivityDone` (`:124-138`) grava **sempre três** chaves num único `multiSet` (`:129-133`) — `done`, `snap`, `ever`; e **condicionalmente uma quarta**, `@ptf_coloring60_finale_seen_<história>`, disparada por `markColoring60FinaleSeen` (`:281-289`) a partir de `ColoringScreen.js:1044-1052`, **somente quando aquele salvamento fecha a história em 3/3 pela primeira vez**. Correção: o desfazimento cirúrgico **existe e é alcançável pelo produto**, cobrindo `done` + `snap`; `ever` sobrevive por desenho; e `finale_seen` **não tem desfazimento cirúrgico algum** — `ColoringScreen.js:1043`: *"Só o reset canônico devolve a primeira vez"* |
| `E7-4` | `:517` | `INJ-09` — *"backup dos **dois** slots (`:385-386`)"* **e**, na mesma linha, *"salvar por cima: gesto proibido · ⚠ desfecho em aberto"* | **Contradição interna de uma linha.** A rede existe e está completa em código: `harnessInject.js:384-386` faz `fazerBackup(a.chave)` **mais** `fazerBackupArquivo` dos *slots* `.a` e `.b`; `restaurarTudo` (`harnessStore.js:392-401`) devolve arquivos **antes** de chaves. O que permanece aberto **não é a capacidade** — é a **autorização**. Registro adicional: `:530` restringe o *"sem rede"* aos salvamentos **fora** de injeção, enquanto `:991` inclui a metade injetada; os dois trechos não se reconciliam |
| `E7-5` | §6.3 | Intervalos citados: `:265-291`, `:292-320`, `:333-347`, `:349-365`, `:377-388`, `:391-404` | **Seis intervalos deslocados.** No harness real: `268-291`, `294-322`, `334-347`, `350-364`, `380-389`, `392-401`. Erro de referência, não de lógica — a leitura de §6.3 permanece válida |
| `E7-6` | `:996` | `ARB-28-ESCOPO` enunciada contra *"`37:297`, `38:25,432-440` (só `#7`) × `06:285` (plural) × `05:2997`"* | **Arbitragem lavrada sobre conjunto de fontes incompleto** — e isso é achado por si. Ficaram de fora: `04:1049-1065` (a tabela dos 17 cenários com coluna `Subportão`, `#1`–`#11` = `SG-A`); `05:1267` (*"`TK-A-081` · §28 **#1–#11** + parcela `SG-A` do **#17**"*); `05:2822-2825` (**Emenda A-12**: *"o roteiro físico final é a UNIÃO, nunca a substituição"*); `07:12-18` (auditoria que declara os cenários **#1, #2, #3, #4, #5, #6, #8, #9, #10, #11** como **faltantes no `06`**); `07:282-297` (§4.1, que os enumera com tasks donas); e `10:37` (única fonte que amarra expressamente os cenários §28 plurais à `R5`). **Nenhuma fonte do corpus afirma "só `#7`"** — `37` e `38` apenas **omitem** |
| `E7-7` | `:992` | *"harness … **85 commits** atrás"* | Medido nesta sessão: `git rev-list --count 1ae353f..07f83c8` = **86**. Também medidos: `1ae353f..521d59c` = **68** e `521d59c..07f83c8` = **18** |
| `E7-8` | `:387` (`P3-10`) | *"Passos de reset do artefato `32` (`E0..E8`) **renomeados** para não colidirem com `E1..E6`"* | **A porta, como escrita, é proibida — e desnecessária.** Proibida: `D-FUND-R2P1-CLOSURE-01` item 7 ([`docs/DECISIONS.md:4127-4129`](../../../docs/DECISIONS.md)) veda *"alterar o `DESIGN` (artefato `32`)"*, e `32:4815` (`G-4`) faz de qualquer mudança do documento um `R2P1_STOP_DESIGN_CUSTODY_DIVERGED`. Desnecessária: a mesma decisão, item 7, proíbe *"executar `RETRY_04`"* **sem exceção**, e o item 5 declara `F6_CLOSURE_BLOCKER = NÃO`, retirando o reset do caminho crítico — logo os passos `E0..E8` do artefato `32` **não são executáveis por ninguém**, e a colisão `X-07` (`:764`) é **nominal, não operacional**. **Reparo correto:** não tocar no artefato `32`; registrar no protocolo de `SF1` que, em `SF1`, `E1..E6` designam **sempre** os extras da `R5` |
| `E7-9` | `:371-372`, `:915` | *"o 'misto' dependeria do salvamento de §28 `#7` (`GH2b`)"* · *"só se `GH2b` for autorizado"* | **Exclusividade incorreta — há pelo menos três origens do misto, não uma.** (a) `GH3` (`:646-654`, passo 5 = *"Salvar"*) também grava sob o Metro do `HEAD`. Cuidado com o mecanismo: **não** é verdade que "todo `writeSlot` carrega os eixos" — `writeSlot` tem dois ramos, e `carryLogicalSchema` só existe no ramo `fmt=2` (`coloring60DrawingStorage.js:419-421` é o ramo `fmt=1`, que não chama nada; `:422-443` é o ramo JSON, com a chamada em `:442`). O que fecha o argumento é o exportador: `ColoringCanvas.js:705-710` emite **sempre** um payload JSON com `logicalW`/`logicalH`, nunca uma *data URL* nua — logo todo salvamento do Colorir 60 cai no ramo `fmt=2` e persiste os eixos. (b) `06:434` traz uma **terceira** origem, que não é injeção nem salvamento: *"Galeria com **acervo misto** após atualizar o app"*. Consequência: a pré-condição de `SF2` só desaparece se **ambas** — `ARB-28-7-TIRO-UNICO` **e** `ARB-OBRA-NOVA` — forem negadas **e** a origem de `06:434` não se materializar. Propaga-se ao ponto `E6-5` de `37:1069` |
| `E7-10` | §7 (`INJ-04`/`INJ-05`) e `08:367-368` | O catálogo trata `INJ-04` (*"o achado mais grave possível"*, `08:246`) e `INJ-05` (seu controle negativo) como injeções efetivas em qualquer circunstância | **Achado novo, não registrado em nenhum artefato: sob o JS do `HEAD` as duas ficam INERTES sobre qualquer ponteiro C60 gravado por esse mesmo JS.** A inércia é **condicional ao ponteiro eleito**, não incondicional — e é exatamente por isso que ela é perigosa. Cadeia verificada elo a elo: `ColoringCanvas.js:705-710` emite `logicalW`/`logicalH` em **todo** `exportPaint`; em `521d59c` a *whitelist* de `writeSlot` (`:376-403`) **não** carregava campo lógico algum; no `HEAD`, `carryLogicalSchema` (`:392`, `:395`) roda em `writeSlot:442` (ramo `fmt=2`) **e** em `resolvePointer60:489`, e os eixos **atravessam** o ponteiro; então `origemBruta` (`ColoringCanvas.js:802-803`) **curto-circuita** devolvendo o bitmap inteiro e **nunca lê `imgW`/`imgH`** — que é exatamente o campo que `INJ-04` corrompe (`harnessInject.js:264`). Resultado sobre um ponteiro do `HEAD`: `INJ-04` produziria **`FAIL` falso** e `INJ-05` um **`PASS` vazio**. Sobre o ponteiro descrito em §4.1 (`rev:22`, sem eixos lógicos) a injeção **morderia normalmente**. **O gatilho que vira a chave está escrito no protocolo:** `08:367-368`, literal — *"**`INJ-04` e `INJ-05` exigem ponteiro `fmt:2`.** Se o harness recusar com essa mensagem, pinte e salve a atividade nesta versão do app **(isso acontece naturalmente em `G5`)** e repita."* — e `alvoC60()` (`harnessInject.js:71`) elege por `mtime` mais recente, com desempate por `rev`. Obedecer a instrução é, sob o `HEAD`, o ato que **torna** a injeção inerte. *(A frase "Sem ele, o `PASS` de `INJ-04` é ambíguo" **não** está em `08:247`, como se poderia supor: ela é do próprio catálogo, `harnessInject.js:284`.)* Consequência de governança: `ARB-JS-BINARIO` decide a **existência** de `INJ-04`/`INJ-05`, e por isso **precede** `ARB-INJ-CAMINHO` |
| `E7-11` | §4.1 (`:185-232`), e os gates que dependem dela — `P3-4` (`:381`) e Bloco 0 passo 10 (`:582`) | §4.1 é titulada *"O acervo do SM-X510 — **perícia, não estimativa**"* e descreve o acervo pelo lacre `TAR-POST03.tar`, **17 033 728 B**; `P3-4` manda conferir *"inventário `INS-01` × §4.1"* com desfecho **"pare"**, e o passo 10 manda *"Conferir contra §4.1: **14 arquivos, 23 chaves, sem `.b.png`**"* | **A referência está superada por pelo menos quatro lacres posteriores, todos medidos e publicados no artefato `35`.** A cadeia de TARs de `35:169-186` abre em `TAR-BE-PRE.tar` = **17 234 944 B** (já **+201 216 B** sobre `TAR-POST03`) e fecha em `CK-D-C17` = `CK-E-C12` = `CK-G-REEXEC-C11` = **17 424 384 B** (**+390 656 B**). No mesmo artefato, `35:44` lavra `CASO 11 (reexecução causal)` = **`PASS`** *"após a correção do esquema"* e `35:45` lavra `CASO 17` — *"Eixos lógicos atravessando o ponteiro"* — = **`PASS`**; `35:269-271` identifica essa correção como o *commit* **`2ffcd82`**; e `36:120-121` declara esses `PASS` *"medidos sobre o binário atual e sobre o `HEAD` atual"*. Ou seja: **houve escrita no acervo depois do lacre que §4.1 descreve, e parte dela sob o JS que grava os eixos lógicos.** Isso **não** prova que a contagem de **arquivos** mudou (crescimento de *bytes* do SQLite não cria arquivo), mas **prova que o conteúdo descrito em `:208-210` não pode ser presumido corrente** — em particular `rev:22` e a ausência dos eixos no ponteiro C60. Como consequência direta, `P3-4` e o passo 10, **na forma escrita, disparariam `pare` contra um acervo legítimo**, ou convidariam o operador a "ajustar" o inventário para o gate passar. **Reparo, sem tocar em §4.1:** tratar §4.1 como *retrato datado do `TAR-POST03`*, não como estado de entrada; e reescrever `P3-4`/passo 10 para comparar `INS-01` contra o **`TAR-R5-ANTES` capturado na própria sessão** (`:579`), registrando §4.1 apenas como referência histórica. **Nada disso é mensurável do *host*: o aparelho está `HANDS OFF`.** |
| `E7-12` | `P3-4` (`:381`) e `INS-01` (`:582`) | O inventário de entrada confere existência e integridade — *"a obra legada do Ateliê **e** o ponteiro C60 de `light`, íntegros; e **não existe** `.b.png`"* | **Falta o único bit que decide se `INJ-04`/`INJ-05` existem.** Por `E7-10`, o que determina se as duas injeções mordem é a **presença de `logicalW`/`logicalH` no ponteiro C60 que `alvoC60()` eleger** — e nenhum gate de entrada pede esse dado. **Reparo:** `INS-01` deve registrar, para o ponteiro eleito, `rev` **e** a presença/ausência dos quatro eixos; e o resultado alimenta `P3-5`. Sem isso, `INJ-04` pode ser executada e classificada como `FAIL` quando o correto seria `NÃO APLICÁVEL` |
| `E7-13` | `§4.3` e `§6.7` (*"O que NÃO tem rede"*) | `§6.7` lista **dois** itens sem rede (salvamentos legítimos fora de injeção; restauração do TAR trancada juridicamente), e `§4.3` justifica sua proibição como válida *"mesmo reversível"* | **Existe um terceiro caminho sem rede, hoje classificado como tendo rede.** `INJ-01` chama `fazerBackupArquivo(a.uriAtivo)` (`harnessInject.js:168`) e **nunca** `fazerBackup(a.chave)` — logo `restaurar(chave)` devolve *"SEM BACKUP — nada a restaurar"* (`harnessStore.js:297`). E `INJ-01` é **a única injeção de `E5` que levanta o painel** (`INJ-02` é a *"ausência honesta"*, `:702-704`). Portanto o único percurso em que o botão **"Pintar de novo" → "Pronto"** pode ser tocado sob observação é justamente o que **não tem rede de chave**: o toque reescreve o ponteiro para o outro *slot* (`coloring60DrawingStorage.js:625`) e apaga o *blob* original (`:657-659`, `if (oldUri && oldUri !== newUri) … deleteBlob(oldUri)`), sem que operação alguma do harness saiba repor o ponteiro — `restaurarArquivo` reporia *bytes* num *slot* que o ponteiro já não referencia. **A proibição de `§4.3` continua correta; a justificativa (*"mesmo reversível"*) não.** Repercute em `ARB-TAR-REDE`, cujo conjunto coberto aumenta |

| `E7-14` | `X-01` (`:758`) e o desdobramento de §28 em `:786-787` | O risco `X-01` nomeia **um** consumidor consuntivo da obra legada — *"§28 `#7` consome a única obra legada"* — e §12 desdobra só `#7`, em `GH2a`/`GH2b` | **Sob `ARB-28-ESCOPO` resolvida como "onze", há DOIS consumidores consuntivos disputando UMA peça.** `§28 #9` — *"Ateliê: abrir obra `v:2` antiga, girar, **salvar**, reabrir"* (`04:1057`, `SG-A`) — exige que a obra **ainda seja `v:2` antiga**, exatamente como `#7` via `TK-A-097` (`05:1013`: *"abrir uma obra salva **antes** da mudança, continuar pintando, **salvar**, reabrir"*). Como o acervo tem **uma só** obra legada (`:210`), o primeiro salvamento a ocorrer esvazia o outro cenário. Os dois enunciados diferem **apenas no gesto de modificação** (pintar × girar), logo há saída que preserva ambos: **fundi-los num salvamento único** — abrir, pintar **e** girar, salvar uma vez, reabrir. **`X-01` precisa ser alargado para nomear `#9`, e `:786-787` precisa da linha de `#9`, antes de `SF1` executar o Bloco 3.** Desenvolvido em `40` §4.2 e §6.2 |

> **O que esta errata NÃO faz.** Não altera o estado de §13 · não converte nenhum `☐` em `PASS` ·
> não responde nenhuma das **seis** arbitragens de §12.1 · não autoriza escrita, *build*, Metro,
> `adb` ou toque no aparelho. O tablet permanece **`HANDS OFF`** e o código permanece **não alterado**.

> **Erratas devidas por outros artefatos** (medidas aqui, **não** lavradas aqui — cabem ao artefato
> dono): `36:97` diz *"26 arquivos, +1704 / −304"* em `src/`, e a medição de `92781ea..2ffcd82`
> restrita a `src/` devolve **25 arquivos, +1702 / −303**; `36:98` diz `coloring60DrawingStorage.js`
> **`+48`**, e o `numstat` real é **`+46 / −2`** (o `48` é *total de linhas tocadas*, convenção usada
> no mesmo período para `AtelierGalleryScreen.js` `306` = `233+73`, mas grafada com sinal `+`);
> `37:9-10` impõe a si mesmo o regime aditivo, e `37:244`/`37:297` reduzem `R5` a `§28 #7` **por
> omissão silenciosa, sem errata**; `23:43-44` afirma que *"nenhum build instalado em qualquer
> aparelho contém hoje a rota (C)"*, texto verdadeiro quando lavrado e hoje **vencido quanto à
> árvore de origem do binário** (`e2702d2` **é** ancestral de `521d59c`, a 8 commits — verificado
> por `git merge-base --is-ancestor`) — mas **não** quanto ao conteúdo empacotado do APK, que
> permanece `APK_BINARY_MANIFEST_NOT_DECODED_WITH_AVAILABLE_TOOL` (`28:328-337`). E `08:181`
(prova **5** do lacre de §4.7) declara *"**PROVADO** — A em `1ae353f`, `git status --porcelain`
**vazio**"*: hoje só a **segunda** conjunção continua verdadeira — o *worktree* `A` está limpo, mas
em **`07f83c8`**, 86 *commits* adiante. A prova 5 precisa ser **reemitida** contra o `HEAD` corrente
antes de ser invocada; as provas 6, 8, 9 e 10 seguem verdadeiras (a 9 agora sobre **623** *commits*).
