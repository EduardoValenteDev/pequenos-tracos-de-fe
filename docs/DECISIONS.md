...
- 🟡 **Mapeamento `F6-UX-01`…`F20-STORE-01` (v6 §9) ↔ `P-01`..`P-167` (matriz canônica).** A matriz continua **única e canônica** por `E018`; o registro da v6 §9 é **vista executiva por fase**. Falta correlacionar código a código. **Arbitragem no F6.1.**
  - ✅ **ENTREGUE em 2026-08-24, no bloco `F6.1` (`ETAPA 7`).** Crosswalk das **24** linhas da v6 §9 em [`docs/roadmap/V6_RISK_CROSSWALK_TO_E018.md`](roadmap/V6_RISK_CROSSWALK_TO_E018.md) e registrado na matriz em **§34**: **17** linhas mapeadas inteiramente em códigos existentes, **5** em parte, **2** sem código anterior; **72** códigos `P-nnn` referenciados, **nenhum** alterado. Sete códigos novos — **`P-170` a `P-176`** — em **acréscimo puro**: a matriz vai de **169** a **176**, com **0 fusões, 0 renumerações e 0 reclassificações**. A matriz **permanece o ledger canônico único**; a v6 §9 permanece **vista executiva**. **A baixa de qualquer item se registra no `P-nnn`.** Nenhum risco foi corrigido — `F6.1` é bloco de baseline. *(O texto acima é preservado: registra fielmente o estado quando foi escrito. Note que a faixa citada era `P-01..P-167`; a matriz já estava em `P-169` desde §33.)*
- 🟡 **Delta de ícone do app** presente na *worktree* principal (branch `fix/loading-performance-foundation`): `assets/icon.png`, `assets/adaptive-icon.png`, `assets/favicon.png`, `app.json` modificados e `assets/branding/beni_icon_source.png` *untracked*. **Fora do F6.0 por instrução explícita do fundador** — não *staged*, não commitado, não descartado, não copiado. **Reconciliação formal no F6.1.**
  - ✅ **ABSORVIDO em 2026-08-24, no bloco `F6.1` (`ETAPAS 5 e 6`), commit `46a469f`.** Os quatro PNGs entraram na linha canônica **byte a byte**, com `SHA256` conferido contra o lacre externo — nenhuma imagem foi reprocessada. O `app.json` recebeu **uma única** alteração autorizada: `android.adaptiveIcon.backgroundColor` de `#7C3AED` para `#FFFFFF`; o *plugin* `withAndroidTabletOrientation` e as demais ocorrências de `#7C3AED` (*splash* e `androidStatusBar`) ficaram **intactos**. `assets/splash-icon.png` **não** foi tocado. A imagem-mestra foi versionada em `assets/branding/beni_icon_source.png` porque a inclusão de asset no binário é decidida pelo grafo de `require()`, não pelo diretório — verificado empiricamente. Portões medidos: `verify:runtime` verde, smoke **4978/4978**, `expo-doctor` **18/18**. **Sem *push*.** A *worktree* principal **não** foi limpa: o delta segue lá, aguardando autorização própria do fundador.
