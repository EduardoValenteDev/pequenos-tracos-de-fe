...
| **8 · Condição de coleta** | Hoje: **só** em `__DEV__` **ou** com `EXPO_PUBLIC_PTF_PERF_TRACE` ligada (`:44-52`) — e essa variável **não é declarada por nenhum dos seis perfis do `eas.json`** (correção declarada em 2026-08-07, abertura da Fase 6: a redação anterior dizia *"cinco perfis"* e omitia `c60-pilot`, acrescentado em 2026-08-01 pelo commit `8464954`; **a substância não muda — nenhum perfil declara a variável**), que é exatamente `P-139`. A Fase 6 habilita em perfil interno; a Fase 9 coleta a linha de base. |
