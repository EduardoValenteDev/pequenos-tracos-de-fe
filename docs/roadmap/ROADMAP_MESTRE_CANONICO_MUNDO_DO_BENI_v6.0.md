...
| F9-C60-REENTRY-01 | P2 produto | F9 | **Achado novo da perna iPhone (`PRODUCT_FINDING_COLORING_REENTRY`).** Entrar no Colorir pelo convite de marco e sair no meio deixa a atividade **sem caminho de volta até a história terminar**. Mecanismo provado, e são duas decisões individualmente deliberadas que se somam: (1) `NarrationScreen` chama `markColoring60MilestoneInviteSeen` no **instante da apresentação**, não na escolha da criança — o convite é estritamente *one-shot*; (2) `CreationColoringJourneySection` é renderizada com `unlocked={isCompleted}` e, travada, os cartões são inertes (`onPress={locked ? undefined : onPress}`). Consequência: o comentário do próprio `Coloring60MilestoneInvite` (“a criança também chega ao Colorir pela jornada da StoryDetail”, “pular aqui não perde nada — só adia”) é **falso enquanto a história está incompleta**. Perde-se o traço parcial da sessão (sair nunca salva) **e** a reentrada; **nenhuma arte já concluída é perdida**. | **Não é `F9-JRN-C60-01`** (aquele trata do reflexo do estado de conclusão; este trata de acesso). Owner **F9** (F9D/F9E), por ser o dono da jornada do Colorir 60 e da cláusula *“sem gate invisível”* da F9E. **`BLOCKS_F6 = NÃO`**: o escopo de Colorir dentro da F6 é o da F6.5 — *“documento lógico estável entre larguras suportadas”* —, geometria, não navegação. Nenhum requisito de roadmap exige reentrada antes da conclusão. |
...
| F12A-CL-01 | P1/P2 | F12A | Criar Livre precisa de saída clara e geometria estável, inclusive teclado. **Instância concreta observada na perna iPhone (`PRODUCT_FINDING_FREE_DRAW_EXIT_CONFIRMATION`)** — este ponto **já estava registrado aqui antes do achado**, e o achado apenas o materializa: o painel de saída de `AtelierCanvasScreen` é gated por plano em `canSaveExit={unlimited && canSave}`. No **Plano Família** o diálogo tem função real (oferece “Salvar e sair”). No **Plano Grátis** ele só avisa: restam “Continuar desenhando” e “Sair sem salvar”, **nenhuma ação que preserve o desenho**. Pior, o grátis **pode** salvar até `ATELIER_FREE_SAVE_LIMIT` pelo botão “Salvar” da barra (gated só por `canSave`), mas o painel de saída esconde “Salvar e sair” mesmo abaixo do limite — as duas superfícies discordam. | Salvar/descartar/cancelar **por plano** e documento lógico consistente — incluindo o *“convite gentil ao responsável no momento de salvar”* que o escopo da F12A já prescreve e que hoje **não existe no caminho de saída**. Owner **F12A**, **`BLOCKS_F6 = NÃO`**. |
...
> **`F6-PERF-01` · ADJUDICAÇÃO DO GAP PELO FUNDADOR (decisão registrada).** O fundador **adjudicou o gap físico Android compacto pós-fix como `ACEITO` por indisponibilidade material do hardware**. A decisão é explícita em não criar prova que não existe.
>
> `F6_PERF_01_STATUS` = **`REMEDIATED_WITH_ACCEPTED_EVIDENCE_GAP`** · `F6_PERF_01_TECHNICAL_REMEDIATION` = **`PASS`** · `F6_PERF_01_COMPACT_PHYSICAL_COUNTERFACTUAL` = **`NOT_TESTED`** · **`BLOCKS_F6 = NÃO`**.
>
> **Preservados, sem conversão:** `COMPACT_POST_FIX_PHYSICAL = NOT_TESTED` · `POST_FIX_COMPACT_ANDROID_PHYSICAL = EVIDENCE_GAP` · `F6-EVID-01 = ACCEPTED_EVIDENCE_GAP`. **Não existe** `COMPACT PASS`, **não existe** `S24 POST-FIX PASS`, **não existe** `CONTRAFACTUAL PROVEN` — e o SM-X510 **não** é usado como falso contrafactual, porque já tinha `Davey! = 0` antes do patch.
>
> **Fundamento da decisão, como o fundador o registrou:** (1) sintoma severo pré-fix observado e objetivamente corroborado no S24 `COMPACT`; (2) superfície e mecanismo provados por log bruto + fonte; (3) patch causal mínimo implementado; (4) *harness* provando 106,8 MB → 7,5 MB no decode dos pins; (5) focados, mutantes, `smoke`, `verify:runtime` e `expo-doctor` verdes; (6) portão visual verde; (7) novo par Android+iOS gerado do mesmo `BUILD_SOURCE_HEAD`; (8) validação física pós-fix no SM-X510 comprovando ausência de regressão e de novo gargalo severo; (9) o SM-X510 não reproduzia o sintoma antes do patch; (10) o S24 ficou indisponível e nenhuma nova prova compacta pode ser produzida nesta etapa.
>
> `F12A-PERF-PARES-01` permanece **separado**, hipótese **não medida**, owner **F12A**, e **não** bloqueia a F6 por analogia.
>
> **Perna tablet encerrada.** `TABLET_CAMPAIGN_FROZEN = SIM` · `TABLET_EVIDENCE_PRESERVED = SIM` (`tablet_CAMPAIGN.log` reconferido: 2.287.838 bytes, `SHA256 = 08448387…3bf56`, tamanho estável, mais os oito arquivos de `perf/`) · `SAFE_TO_DISCONNECT_TABLET = SIM` — nenhum processo de captura depende do aparelho. **Nenhuma outra ação física Android será solicitada.**

> **`F6-IOS-01` · PREPARAÇÃO DA PERNA IPHONE (campanha humana ainda NÃO iniciada).** Bloco de preparação apenas: identidade, proveniência, provisionamento, método de instalação e rota humana. **Nenhuma ação humana foi pedida nesta execução.**
>
> **IPA canônico.** `IOS_EAS_ID = da48afb6-11c7-4955-81cc-d9eef4c42b6f` · `status = FINISHED` · `profile = preview` · `distribution = INTERNAL` · versão `1.0.0` · `buildNumber 1` · `isForIosSimulator = false` · `IOS_BUILD_SOURCE_HEAD = 645e6275c94b2061aa424e49cdc5edfec8ba8c7c` (confirmado no `gitCommitHash` do EAS). Artefato: `https://expo.dev/artifacts/eas/DASvn_2R2vP6M5ybCCLNLDdgM2W84sAfJqraNGx2gm0.ipa` (154.407.203 bytes). O IPA de `2dcf717` permanece **`SUPERSEDED`**.
>
> **Prazo real do artefato.** `expirationDate = 2026-09-08T17:32:50Z` — **≈ 14 dias** a partir da geração. Passado esse prazo o artefato deixa de ser servido e um novo build seria necessário, obrigatoriamente do mesmo `BUILD_SOURCE_HEAD 645e627`. **Este é o único relógio correndo sobre a perna iPhone.**
>
> **`IOS_DEVICE_PROVISIONING = READY` — provado, não presumido.** O perfil embutido no próprio IPA foi extraído do artefato remoto por requisições `Range` (sem baixar os 154 MB) e lido: `Payload/Beni.app/embedded.mobileprovision`, perfil **`*[expo] com.valentedev.pequenostracosdefe AdHoc 1783548237486`**, `UUID 66264420-3b18-47ba-aa50-32f64b3c219d`, time `Eduardo Valente (UPVJN674JW)`, criado em `2026-08-07T21:04:36Z`, **válido até `2027-07-08T21:55:20Z`**, `get-task-allow = false`. O bloco `ProvisionedDevices` traz **exatamente dois UDIDs**: `00008110-001C75E214E3A01E` (iPhone) e `00008101-000E7C811E43A01E` (iPad). O `eas device:list` do time `UPVJN674JW` confirma os mesmos dois aparelhos, ambos `enabled`, com o iPhone registrado em `2026-07-08`. **O iPhone do fundador está incluído no perfil que assina este IPA** — nenhum rebuild por provisionamento é necessário.
>
> **`IOS_INSTALL_WITHOUT_USB = SIM`.** Distribuição `INTERNAL` com perfil **Ad Hoc** e certificado `iPhone Distribution` permite instalação *over-the-air* pela página do build. `IOS_USB_INSTRUMENTATION = INDISPONÍVEL` (o fundador não possui cabo compatível) — **nada na rota depende de USB, Xcode, `ios-deploy` ou console físico**.
>
> **Consequência honesta e incômoda: o patch do `F6-PERF-01` é INERTE no iOS.** `resizeMethod` é declarado `@platform android` no RN 0.81 (`Libraries/Image/ImageProps.js:116`) e `resizeMultiplier` só atua quando `resizeMethod = resize`; o iOS não usa Fresco. Portanto **a perna iPhone NÃO valida o patch e NÃO fecha o gap Android compacto** — ela valida o **contrato portrait**, o **layout compacto** e a **ausência de regressão funcional no iOS**. Registrar o contrário seria fabricar equivalência.
>
> **O que se sabe do aparelho sem cabo.** `deviceClass = IPHONE` (EAS) e prefixo de UDID `00008110` = SoC Apple **T8110 (A15 Bionic)** — família iPhone 13 / 13 mini / 13 Pro / 13 Pro Max / 14 / 14 Plus / SE (3ª geração). **Todas** essas larguras (375-428 pt) são **`COMPACT`**, então a perna iPhone é de fato um teste de layout compacto. **O modelo exato, a versão do iOS e o estado do bloqueio de orientação NÃO podem ser derivados remotamente** e precisam ser lidos pelo fundador no próprio aparelho — ficam como `UNKNOWN` até lá, sem chute.
>
> **Ressalva metodológica para o objetivo "ausência de landscape".** Se o **bloqueio de orientação do iOS estiver LIGADO**, o portrait é imposto pelo sistema e **não** pelo app: o teste passaria por motivo errado. A rota humana precisa **desligar o bloqueio de orientação antes** de testar resistência a landscape, ou o resultado é vazio.
>
> **`IOS_NATIVE_LOG_EVIDENCE = EVIDENCE_GAP`.** Sem cabo não há `idevicesyslog`, `Console.app` nem `os_log`. **Nenhuma equivalência com o logcat Android será fabricada** — a evidência do iPhone será atestação humana e, se houver, captura de tela; `VIDEO_EVIDENCE` não é obrigatório.
>
> **Rota humana preparada (ainda não solicitada).** (0) confirmar modelo, versão do iOS e bloqueio de orientação; (1) abrir no **Safari do iPhone** a página do build `https://expo.dev/accounts/eduardocriacao/projects/pequenos-tracos-de-fe/builds/da48afb6-11c7-4955-81cc-d9eef4c42b6f` e tocar em **Install**; (2) confirmar o diálogo do iOS e aguardar o ícone **Beni** na tela de início — como o build é Ad Hoc assinado com certificado de distribuição, **não deve** haver etapa de confiança manual; se ainda assim o iOS reclamar de desenvolvedor não confiável, o caminho é *Ajustes → Geral → Gerenciamento de VPN e Dispositivo*; (3) rota funcional Home → Aventuras → A Criação → cena → Colorir → Criar Livre → retorno Home, observando contrato portrait, layout compacto, ausência de landscape, ausência de crash e percepção de performance. **Defeitos conhecidos de onboarding/tour da F7 não são confundidos com F6**, salvo se bloquearem a rota.
>
> `READY_TO_INSTALL_IPHONE = SIM` · `READY_FOR_IPHONE_CAMPAIGN = SIM` — a campanha só começa mediante instrução do fundador.

> **F6.9 · PERNA IPHONE — `PASS` funcional; evidência nativa `EVIDENCE_GAP`.** Aparelho **declarado pelo fundador**, não inferido do UDID: **iPhone 14**, **iOS 26.6**, UDID `00008110-001C75E214E3A01E`. `ORIENTATION_LOCK_INITIAL_STATE = OFF` e `ORIENTATION_LOCK_TEST_STATE = OFF` — o bloqueio de orientação **já estava desligado** antes da campanha e nenhuma outra configuração do sistema foi alterada. Instalação *over-the-air* pela página do build `da48afb6-11c7-4955-81cc-d9eef4c42b6f` (`preview`, `distribution INTERNAL`, Ad Hoc, `IOS_BUILD_SOURCE_HEAD 645e6275c94b2061aa424e49cdc5edfec8ba8c7c`), sem USB, sem Xcode e sem etapa de confiança manual. **Nenhum IPA antigo foi usado.**
>
> **Atestação humana (todos os valores fornecidos pelo fundador — nada aqui foi medido por instrumentação):** `APP_FUNCTIONAL = PASS` · `CRASH = NÃO` · `WHITE_SCREEN = NÃO` · `FREEZE = NÃO` · `UNEXPECTED_EXIT = NÃO` · `RANDOM_FAILURE = NÃO` · `PORTRAIT_CONTRACT = PASS` · `LANDSCAPE_RELAYOUT = NÃO` · `COMPACT_LAYOUT = PASS`. As **sete superfícies** da rota — Home, Aventuras/Mapa, StoryDetail, cena narrativa, Colorir com o Beni, Criar Livre e retorno ao início — deram **`PASS`**. As **quatro perguntas de percepção** foram respondidas **`RÁPIDO`/`FLUIDO`**.
>
> **Ressalvas registradas e não convertidas em prova.** (1) `IOS_USB_INSTRUMENTATION = INDISPONÍVEL` e `IOS_NATIVE_LOG_EVIDENCE = EVIDENCE_GAP` — sem `idevicesyslog`, `Console.app` ou `os_log`, e **nenhuma equivalência com o logcat Android é fabricada**. (2) `VIDEO_EVIDENCE = AUSENTE` — a evidência é a atestação direta do fundador. (3) **Percepção humana não vira benchmark técnico:** sem log nativo não existe métrica de frame, de *janky* ou de upload de bitmap nesta perna, e nenhuma foi inventada. (4) **O patch do `F6-PERF-01` continua INERTE no iOS** (`resizeMethod` é `@platform android`): esta perna **não** valida o patch e **não** fecha o gap Android compacto. (5) O `portrait` do app já é declarado no `Info.plist`, então `LANDSCAPE_RELAYOUT = NÃO` confirma que o binário se comporta **como declarado** — não é teste discriminante de layout.
>
> **Dois achados de produto surgiram durante a rota**, ambos classificados na matriz de riscos: `F9-C60-REENTRY-01` (owner **F9**) e a instância concreta de `F12A-CL-01` (owner **F12A**). **Nenhum dos dois impediu materialmente a execução da rota** e **nenhum dos dois é F6.** Defeitos conhecidos de onboarding/tour da F7 não foram classificados como F6.

> **RECONCILIAÇÃO FINAL DA F6 · matriz física, critérios de saída e gate de lacre.**
>
> **Matriz física final.** Nenhuma linha empresta resultado para outra e nenhum resultado é convertido em prova de aparelho não testado.
>
> | Perna | Aparelho | Banda | Binário | Resultado |
> | --- | --- | --- | --- | --- |
> | Tablet pós-fix | SM-X510, Android 16, 823 dp, font scale **1,15** | `MEDIUM`/`EXPANDED` (sidebar) | APK canônico `645e627` | `PASS` funcional + logcat sem `FATAL`/`ANR`/tombstone |
> | Telefone Android | SM-S928B, Android 15, 360 dp, font scale **1,0** | `COMPACT` (barra inferior) | APK da F6.8 `2dcf717` — **pré-fix** | funcional `PASS`; performance **disparou** `F6-PERF-01` |
> | Telefone Android **pós-fix** | — | `COMPACT` | — | **`NOT_TESTED`** — aparelho materialmente indisponível |
> | iPhone | iPhone 14, iOS 26.6, Dynamic Type padrão | `COMPACT` | IPA canônico `645e627` | `PASS` funcional; **sem log nativo** |
>
> O **SM-X510 não é contrafactual compacto** — já tinha `Davey! = 0` **antes** do patch, logo não pode demonstrar a ausência de um defeito que nunca reproduziu. O **iPhone não substitui** o telefone Android pós-fix, porque o patch é inerte no iOS. `COMPACT_POST_FIX_PHYSICAL` permanece **`NOT_TESTED`**.
>
> **`SAME_SOURCE_HEAD` — provado por documentação existente, sem gerar build novo.** Ver `SAME_SOURCE_HEAD_PROOF` acima: o `gitCommitHash` do EAS é idêntico nas duas plataformas e igual a `645e6275c94b2061aa424e49cdc5edfec8ba8c7c`. O APK instalado no tablet e no telefone e o IPA instalado no iPhone descendem do **mesmo `BUILD_SOURCE_HEAD`**, que permanece **imutável**.
>
> **`F6-PERF-01` — reconciliação, sem reabertura.** O item **já foi adjudicado** pelo fundador (bloco `ADJUDICAÇÃO DO GAP`): `F6_PERF_01_STATUS = REMEDIATED_WITH_ACCEPTED_EVIDENCE_GAP`, `F6_PERF_01_TECHNICAL_REMEDIATION = PASS`, `F6_PERF_01_COMPACT_PHYSICAL_COUNTERFACTUAL = NOT_TESTED`, **`BLOCKS_F6 = NÃO`**. A perna iPhone **não altera nada disso** e não produz `PASS` compacto pós-fix.
>
> **Critérios formais de saída da F6, um a um.**
>
> | Critério (`SAÍDA F6`) | Veredito | Prova |
> | --- | --- | --- |
> | Nenhum P0/P1 F6 em portrait | **`BLOCKED`** | `F6-TYPE-01` mantém residual físico não quitado — detalhado abaixo |
> | Tablet sem colisão com sidebar/UI do sistema | **`PASS`** | `F6-UX-01` fechado com prova física do fundador no SM-X510 em retrato (`F6_2_COMPLETE = SIM`, `F6.2 = PASS`, commit `28aba98`), confirmado de novo pela campanha da perna tablet |
> | Performance release like aceitável | **`ACCEPTED_GAP`** | `F6-PERF-01` adjudicado pelo fundador; remediação técnica `PASS`, contrafactual compacto `NOT_TESTED`, `BLOCKS_F6 = NÃO` |
> | Android e iOS canônicos e equivalentes | **`PASS`** | `F6-BUILD-01` fechado: par de builds do mesmo `645e627`, `SAME_SOURCE_HEAD_PROOF`, ambos instalados e executados em aparelho físico, IPA antigo marcado `SUPERSEDED` |
...
>
> **Este residual não é adjudicado por conta própria como `ACCEPTED_GAP`.** Diferente de `F6-PERF-01` e `F6-EVID-01` — que o fundador adjudicou explicitamente **porque o hardware ficou materialmente indisponível** —, aqui **existe aparelho disponível** para quitar. Convertê-lo em `PASS` seria exatamente o *“`PASS` fictício”* que a própria `SAÍDA F6` proíbe.
>
