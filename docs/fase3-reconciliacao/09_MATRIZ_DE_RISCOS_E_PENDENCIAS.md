...
> **Atualizado nas Fases 4A, 4B, 4C, 4D e 4E (Product Lock), na Fase 5 (§29, §30), na abertura da Fase 6 (§31 — duas correções de numeral declaradas, nenhum risco alterado), no Delta v4.1 da Fase 6 (§32 — dezoito códigos novos `P-150` a `P-167`, acréscimo puro, nenhuma renumeração e nenhuma reclassificação de código anterior) e na auditoria investigativa pós validação física da Fase 6 (§33 — dois códigos novos `P-168` e `P-169`, acréscimo puro, duas células de *Observação* alteradas com declaração explícita, nenhuma renumeração) e no bloco `F6.1` da Fase 6 (§34 — sete códigos novos `P-170` a `P-176` vindos do crosswalk da `v6` §9, acréscimo puro, nenhuma renumeração, nenhuma reclassificação e nenhuma célula alterada fora das linhas novas).**
...
| **Total de riscos** | **176** — `P-01` a `P-131` (herdados) + `P-132` a `P-134` (E016, ETAPA 4) + `P-135` a `P-139` (E018, ETAPA 2) + `P-140` (Fase 4A, §24) + `P-141` a `P-148` (Fase 4D, §27) + `P-149` (Fase 5, §29) + `P-150` a `P-167` (Fase 6 · Delta v4.1, §32) + `P-168` e `P-169` (Fase 6 · auditoria investigativa pós validação física, §33) + `P-170` a `P-176` (Fase 6 · `F6.1`, crosswalk da `v6` §9, §34). **A Fase 4E (§28) e a abertura da Fase 6 (§31) não criaram nenhum código novo.** |
| **Fusões realizadas** | **0** |
| **Renumerações** | **0** — `P-140`, `P-141` a `P-148`, `P-149`, `P-150` a `P-167`, `P-168` a `P-169` e `P-170` a `P-176` são acréscimos puros |
...
> árbitro das **decisões de produto**; a **`v6.0`** governa o **roadmap e a sequência
> de fases** (papel que era da `v5` até 2026-08-24, ver §34); este artefato governa **inventário, identidade, status, fase e rastreabilidade
> das pendências**.
...
| **P-172** | Bordas e sombras inconsistentes no sistema compartilhado de cards e conquistas | Cards e conquistas apresentam bordas cinzas e sombras inconsistentes entre si. A `v6` determina investigar o componente compartilhado **antes** de qualquer correção local, o que caracteriza causa comum ainda não isolada | **UI E RESPONSIVIDADE** | `COMPROVADO PELO DOCUMENTO` · `ROADMAP_MESTRE_CANONICO_MUNDO_DO_BENI_v6.0.md` §9 | Fase 6 · `F6.1` crosswalk da `v6` §9 (§34) | `ABERTO` | ND | **MÉDIO** | cards em geral, Estrelinhas, conquistas | inconsistência visual percebida como acabamento inacabado | provável divergência de token ou de componente base, não isolada | 6 | 6 | - | 6 | `VFP` | `NÃO BLOQUEIA PRODUCT LOCK` | `NÃO BLOQUEIA LANÇAMENTO` | ORIGEM EXECUTIVA: `F6-VIS-01`. NÃO É `P-33`, que trata da borda do Beni no *onboarding* e pertence à Fase 7 | `COMPROVADO PELO DOCUMENTO` | CRIADO EM `F6.1` (§34) pelo crosswalk da `v6` §9 para o ledger canônico, sem renumerar nem reclassificar nenhum código anterior. Ver [`docs/roadmap/V6_RISK_CROSSWALK_TO_E018.md`](../roadmap/V6_RISK_CROSSWALK_TO_E018.md). Risco NÃO corrigido em `F6.1`, que é bloco de baseline. A `v6` proíbe explicitamente correções locais antes da investigação do componente compartilhado. |
...
| **P-171** | Mapa exibe marcador de inicial mesmo quando o artwork existe | O mapa apresenta o marcador textual de inicial da história em lugar da arte, mesmo em histórias cujo artwork está presente. A cadeia `source` / `fallback` / cache que decide a exibição não está resolvida nem coberta por nenhum código anterior | **ASSETS** · sec.: UI E RESPONSIVIDADE | `COMPROVADO PELO DOCUMENTO` · `ROADMAP_MESTRE_CANONICO_MUNDO_DO_BENI_v6.0.md` §9 | Fase 6 · `F6.1` crosswalk da `v6` §9 (§34) | `ABERTO` | ND | **ALTO** | Mapa das Aventuras | a criança vê uma letra no lugar da ilustração da história e perde a leitura visual do mapa | resolução de fonte de imagem, *fallback* e cache sem regra única declarada | 6 | 6 | `P-105`, `P-111`, `P-26` | 6 | `VFP` | `NÃO BLOQUEIA PRODUCT LOCK` | `PODE BLOQUEAR LANÇAMENTO` | ORIGEM EXECUTIVA: `F6-MAP-01`. VIZINHOS QUE NÃO SÃO O MESMO FATO: `P-105` (chave sem `require`), `P-111` (*ratio* de capa), `P-123` (capa declarada em *pack*), `P-26` (cegueira a *pack* ausente) | `COMPROVADO PELO DOCUMENTO` | CRIADO EM `F6.1` (§34) pelo crosswalk da `v6` §9 para o ledger canônico, sem renumerar nem reclassificar nenhum código anterior. Ver [`docs/roadmap/V6_RISK_CROSSWALK_TO_E018.md`](../roadmap/V6_RISK_CROSSWALK_TO_E018.md). Risco NÃO corrigido em `F6.1`, que é bloco de baseline. A saída esperada declarada pela `v6` exige testar as 20 histórias; a prova é física e ainda não existe. |
...
| **P-174** | Cadê a Ovelhinha depende de mudança de estado ou de rotação para inicializar | A entrada do jogo já apresentou inicialização dependente de mudança de estado ou de rotação, em vez de começar corretamente em retrato e concluir sem gatilho externo. Este é o código canônico que faltava para o achado `E-01` da auditoria investigativa da Fase 6 — a `ExpoImage` da ovelha que nunca recarrega entre rodadas, travando o gate de prontidão da rodada 2 em diante | **JOGOS** | DOCUMENTADO HISTORICAMENTE · §33.5 (achado `E-01`, confiança declarada de 95%) · `COMPROVADO PELO DOCUMENTO` · `ROADMAP_MESTRE_CANONICO_MUNDO_DO_BENI_v6.0.md` §9 | Fase 6 · `F6.1` crosswalk da `v6` §9 (§34) | `ABERTO` | ND | **CRÍTICO** | Cadê a Ovelhinha | no Modo Fácil, que é o padrão, a partida seguinte não inicia e a criança fica presa | prontidão de cena acoplada a evento externo de estado ou de rotação | 12A | 12A | `P-152` | 12A | `VFP` | `NÃO BLOQUEIA PRODUCT LOCK` | `BLOQUEIA LANÇAMENTO` | ORIGEM EXECUTIVA: `F12A-OV-01`. ACHADO DA AUDITORIA: `E-01`. NÃO É `P-152`, que é a camada genérica de `resize` | DOCUMENTADO HISTORICAMENTE | CRIADO EM `F6.1` (§34) pelo crosswalk da `v6` §9 para o ledger canônico, sem renumerar nem reclassificar nenhum código anterior. Ver [`docs/roadmap/V6_RISK_CROSSWALK_TO_E018.md`](../roadmap/V6_RISK_CROSSWALK_TO_E018.md). Risco NÃO corrigido em `F6.1`, que é bloco de baseline. FECHA UMA LACUNA DE GOVERNANÇA DECLARADA: a §33.2 e a §33.5 registraram que `E-01` era **bloqueador crítico de lançamento sem código `P` próprio**, porque o Human Gate de 2026-08-10 autorizou exatamente dois códigos novos. A `v6` §9 aprovou `F12A-OV-01`, e `F6.1` converte a lacuna em código. A fase proprietária continua sendo a **12A** e a Fase 6 permanece **PROIBIDA** de corrigir. "Cadê a Ovelhinha" segue DEV-GATED / EM TESTE. Nenhum código anterior foi reclassificado por isto. |
...
| **P-175** | Nenhum *threat model* nem inspeção do artefato final antes do release | Não existe processo declarado de *threat model*, varredura de segredos e inspeção do binário gerado antes da publicação. Os códigos anteriores cobrem superfícies internas alcançáveis e declarações incorretas ao responsável, mas nenhum cobre a inspeção do **artefato final** | **PRIVACIDADE** · sec.: FERRAMENTAS INTERNAS | `COMPROVADO PELO DOCUMENTO` · `ROADMAP_MESTRE_CANONICO_MUNDO_DO_BENI_v6.0.md` §9 | Fase 6 · `F6.1` crosswalk da `v6` §9 (§34) | `ABERTO` | ND | **ALTO** | artefato de release | segredo, ferramenta interna ou dado infantil pode ser publicado dentro do binário | ausência de porta de inspeção *fail closed* antes da publicação | 19 | 19 | `P-55`, `P-107`, `P-115`, `P-92`, `P-149` | 19 | `NÃO SE APLICA` | `NÃO BLOQUEIA PRODUCT LOCK` | `BLOQUEIA LANÇAMENTO` | ORIGEM EXECUTIVA: `F19-SEC-01`. MESMO BLOCO DE CORREÇÃO: `P-55`, `P-107`, `P-115` | `COMPROVADO PELO DOCUMENTO` | CRIADO EM `F6.1` (§34) pelo crosswalk da `v6` §9 para o ledger canônico, sem renumerar nem reclassificar nenhum código anterior. Ver [`docs/roadmap/V6_RISK_CROSSWALK_TO_E018.md`](../roadmap/V6_RISK_CROSSWALK_TO_E018.md). Risco NÃO corrigido em `F6.1`, que é bloco de baseline. Escopo estritamente processual: este código não afirma que exista vazamento hoje, e sim que **não existe a porta que provaria o contrário**. |
...
| **P-170** | Nenhum vínculo verificável entre binário instalado e commit de origem | Não existe artefato que ligue um binário físico já instalado ao `commit`, à árvore, ao perfil de build e ao manifesto de conteúdo que o produziram. O iPhone atualmente instalado é o caso concreto: representa build antigo com conteúdo divergente e sua proveniência não é demonstrável a partir de nenhum registro versionado | **DEPENDÊNCIAS E BUILD** · sec.: DOCUMENTAÇÃO | `COMPROVADO PELO DOCUMENTO` · `ROADMAP_MESTRE_CANONICO_MUNDO_DO_BENI_v6.0.md` §9 | Fase 6 · `F6.1` crosswalk da `v6` §9 (§34) | `ABERTO` | ND | **ALTO** | governança de build, campanha física, evidência | nenhum impacto direto sobre a criança | toda campanha física corre o risco de validar um artefato cuja origem não é demonstrável, e um `PASS` pode ser atribuído ao commit errado | 6 | 6 | `P-94`, `P-137` | 6 | `VFP` | `NÃO BLOQUEIA PRODUCT LOCK` | `PODE BLOQUEAR LANÇAMENTO` | ORIGEM EXECUTIVA: `F6-BUILD-01`. MESMO BLOCO DE CORREÇÃO: `P-94`, `P-137` | `COMPROVADO PELO DOCUMENTO` | CRIADO EM `F6.1` (§34) pelo crosswalk da `v6` §9 para o ledger canônico, sem renumerar nem reclassificar nenhum código anterior. Ver [`docs/roadmap/V6_RISK_CROSSWALK_TO_E018.md`](../roadmap/V6_RISK_CROSSWALK_TO_E018.md). Risco NÃO corrigido em `F6.1`, que é bloco de baseline. O contrato de proveniência e o registro histórico de builds foram criados em `F6.1`; o risco só é fechado quando existir build canônico gerado sob esse contrato, o que pertence a `F6.8`. |
...
| **P-173** | Gap histórico de evidência: `raw.log` de `R1-PEND-5` não preservado e irrecuperável | O `raw.log` correspondente a `R1-PEND-5` não foi preservado e é irrecuperável. A `v6` determina não fabricar substituto, registrar o gap e **não** repetir a campanha em paisagem apenas para recriá-lo | **OUTRO** · sec.: DOCUMENTAÇÃO | `COMPROVADO PELO DOCUMENTO` · `ROADMAP_MESTRE_CANONICO_MUNDO_DO_BENI_v6.0.md` §9 | Fase 6 · `F6.1` crosswalk da `v6` §9 (§34) | `GAP HISTÓRICO IRRECUPERÁVEL` | ND | **INFORMATIVO** | evidência de campanha física | nenhum impacto sobre a criança | uma evidência de campanha não existe e não pode ser reconstituída sem fabricação | 6 | 6 | - | - | `NÃO SE APLICA` | `NÃO BLOQUEIA PRODUCT LOCK` | `NÃO BLOQUEIA LANÇAMENTO` | ORIGEM EXECUTIVA: `F6-EVID-01` | `GAP DECLARADO` | CRIADO EM `F6.1` (§34) pelo crosswalk da `v6` §9 para o ledger canônico, sem renumerar nem reclassificar nenhum código anterior. Ver [`docs/roadmap/V6_RISK_CROSSWALK_TO_E018.md`](../roadmap/V6_RISK_CROSSWALK_TO_E018.md). Risco NÃO corrigido em `F6.1`, que é bloco de baseline. Este código existe para que o gap seja **rastreável**, não para que seja fechado. Fechá-lo exigiria fabricar evidência, o que a `v6` proíbe. Permanece aberto por natureza. |
| **P-176** | Identidade legal, contas de loja e formulários sem reconciliação | Contas de loja, `DUNS`, identidade legal da titularidade e formulários de publicação podem divergir entre si e em relação à empresa. Nenhum código anterior cobre o domínio jurídico-comercial de loja | **OUTRO** · sec.: DEPENDÊNCIAS E BUILD | `COMPROVADO PELO DOCUMENTO` · `ROADMAP_MESTRE_CANONICO_MUNDO_DO_BENI_v6.0.md` §9 | Fase 6 · `F6.1` crosswalk da `v6` §9 (§34) | `ABERTO` | ND | **ALTO** | publicação nas lojas | nenhum impacto direto sobre a criança | divergência entre titularidade declarada, contas de desenvolvedor e formulários pode impedir ou reverter a publicação | 20 | 20 | - | 20 | `NÃO SE APLICA` | `NÃO BLOQUEIA PRODUCT LOCK` | `BLOQUEIA LANÇAMENTO` | ORIGEM EXECUTIVA: `F20-STORE-01` | `COMPROVADO PELO DOCUMENTO` | CRIADO EM `F6.1` (§34) pelo crosswalk da `v6` §9 para o ledger canônico, sem renumerar nem reclassificar nenhum código anterior. Ver [`docs/roadmap/V6_RISK_CROSSWALK_TO_E018.md`](../roadmap/V6_RISK_CROSSWALK_TO_E018.md). Risco NÃO corrigido em `F6.1`, que é bloco de baseline. Depende da trilha de CNPJ e titularidade descrita na `v6` §11, que corre em paralelo desde a Fase 6. |
...
## 34. Fase 6 · `F6.1` — sete códigos novos vindos do crosswalk da `v6` §9 (2026-08-24)

> **Natureza desta seção.** Registro de **acréscimo puro**. Nasce da `ETAPA 7` do bloco `F6.1`,
> que reconciliou o **registro executivo de defeitos** da
> [`v6` §9](../roadmap/ROADMAP_MESTRE_CANONICO_MUNDO_DO_BENI_v6.0.md) com este ledger canônico.
> O documento de reconciliação é
> [`docs/roadmap/V6_RISK_CROSSWALK_TO_E018.md`](../roadmap/V6_RISK_CROSSWALK_TO_E018.md).
>
> **Nenhum código de `P-01` a `P-169` foi apagado, fundido, renumerado ou reclassificado.**
> **0 fusões · 0 renumerações · 0 reclassificações · 0 células alteradas fora das linhas novas.**
> A matriz passa de **169** para **176**.
>
> **Esta seção não corrige nenhum defeito.** `F6.1` é bloco de baseline, identidade de build e
> absorção de ícone — não de correção de produto.

### 34.1 A questão de autoridade, resolvida sem criar ledger concorrente

A `v6` §9 introduziu um esquema de códigos executivos por fase (`F6-UX-01` … `F20-STORE-01`),
enquanto E018 estabeleceu **este artefato** como a **única fonte canônica de riscos**. A `v6`
**não** declara esta matriz *superseded*, e a §17.1 da `v6` **não** a lista entre os documentos
rebaixados.

**Resolução:** a `v6` §9 é **camada de leitura executiva por fase**; este artefato continua
governando **inventário, identidade, status, fase e rastreabilidade**. **A baixa de qualquer item
se registra no `P-nnn`, nunca no código executivo.**

### 34.2 A prova de que os dois esquemas descrevem o mesmo universo

Esta matriz **já continha**, antes da `v6`, aliases escritos no formato executivo. Quatro códigos
da `v6` §9 são redescrições **literais** de aliases pré-existentes — e por isso **não** geraram
código novo:

| Código `v6` | Alias literal já registrado aqui | Código canônico |
|---|---|---|
...
Das **24** linhas da `v6` §9, **17** foram mapeadas inteiramente em códigos existentes, **5** em
parte, e **2** não tinham nenhum código. Os **72** códigos `P-nnn` referenciados pelo crosswalk
seguem **inalterados**.

### 34.3 Por que cada código novo existe — confronto contra `P-01` a `P-169`

Aplicada a mesma regra do fundador registrada em §27.2, §32.1 e §33.1: **não se cria código por
simples ampliação de evidência**. Cada um dos sete nasceu de **busca negativa declarada** no corpo
inteiro da matriz, não de semelhança de título.

| Novo | Origem `v6` | Busca negativa declarada | Código anterior mais próximo e por que **não** é o mesmo fato |
|---|---|---|---|
| `P-170` | `F6-BUILD-01` | *proveniência*, *provenance* e *manifesto de build* **não ocorrem** na matriz | `P-94` é a ausência de `runtimeVersion`/`updates`/`assetBundlePatterns`; `P-137` é o `appVersion` literal. Nenhum enuncia a **inexistência de vínculo verificável entre binário instalado e commit de origem** |
| `P-171` | `F6-MAP-01` | nenhuma linha descreve o mapa exibindo marcador de inicial com artwork presente | `P-105` é chave sem `require`; `P-111` é *ratio* de capa; `P-123` é capa declarada em *pack*; `P-26` é cegueira a *pack* ausente. O defeito é de **resolução `source`/`fallback`/cache no mapa** |
| `P-172` | `F6-VIS-01` | *shadow* e *sombra* **não ocorrem** na matriz | `P-33` é a borda do Beni no *onboarding*, natureza ASSETS e **Fase 7**. O fato novo é o **sistema compartilhado de cards e conquistas** |
| `P-173` | `F6-EVID-01` | `raw.log` e `R1-PEND` **não ocorrem** na matriz | nenhum. O gap de evidência histórica nunca havia sido codificado |
| `P-174` | `F12A-OV-01` | nenhuma linha da Ovelhinha descreve inicialização dependente de estado ou rotação | `P-72`, `P-75`, `P-76`, `P-77` e `P-79` são conteúdo, *docblock*, cena e *reduce-motion*; `P-152` é a camada **genérica** de `resize`. Ver §34.4 |
| `P-175` | `F19-SEC-01` | *threat model*, *scan* e *secret* **não ocorrem** na matriz | `P-55`, `P-107`, `P-115`, `P-92` e `P-149` cobrem superfícies internas alcançáveis e declarações incorretas ao responsável, **não** a inspeção do **artefato final** |
| `P-176` | `F20-STORE-01` | *DUNS*, *CNPJ* e *titularidade* **não ocorrem** na matriz | nenhum. O domínio jurídico-comercial de loja não possuía código |

### 34.4 `P-174` fecha uma lacuna de governança **declarada** em §33

A §33.2 e a §33.5 registraram, com todas as letras, que o achado `E-01` — **a `ExpoImage` da
ovelha que nunca recarrega entre rodadas, travando o gate de prontidão da rodada 2 em diante, em
100% das partidas do Modo Fácil** — era **bloqueador crítico de lançamento sem código `P` próprio**,
porque o Human Gate de 2026-08-10 autorizou exatamente dois códigos novos e a §33.5 declarou:
*"Não inventei um terceiro."*

A `v6` §9 aprovou `F12A-OV-01`, que enuncia esse mesmo fato. `F6.1` **converte a lacuna declarada em
código canônico**, que é precisamente o caminho previsto pela §33.2: *"serão convertidos em código —
se for o caso — pela fase proprietária de cada um, sob autorização própria"*, aqui exercida pela
aprovação da `v6`.

**O que NÃO mudou:** a fase proprietária continua sendo a **12A**; a Fase 6 permanece **PROIBIDA**
de corrigir; *"Cadê a Ovelhinha"* segue **DEV-GATED / EM TESTE**; a classificação de lançamento
permanece `BLOQUEIA LANÇAMENTO`. Os achados `E-02` a `E-05` continuam **congelados para `F12A` sem
código próprio**, e **isso segue sendo pendência de governança aberta** — não foram convertidos aqui
porque a `v6` §9 não os aprovou individualmente. **Declaro a lacuna remanescente em vez de
preenchê-la.**

### 34.5 Estado de evidência dos sete — declarado sem inflação

Seis dos sete entram com evidência **documental**, não de código: sua prova é a
[`v6`](../roadmap/ROADMAP_MESTRE_CANONICO_MUNDO_DO_BENI_v6.0.md) §9 aprovada, e **nenhum arquivo
executável foi lido para sustentá-los**. `P-174` é o único que herda evidência mais forte, por
apoiar-se na auditoria investigativa da §33.5 (confiança declarada de 95%).

**Consequência declarada:** quando a fase proprietária de cada um for executada, a **primeira**
obrigação é reconferir o fato contra o código e **promover ou rebaixar** o estado de evidência. Um
código com evidência documental **não** é prova de defeito no código — é prova de que o defeito foi
**aprovado como real pelo fundador** e precisa de dono.

### 34.6 Recontagem — declarada campo a campo

| Campo | Antes (§33.6) | Depois | Delta |
|---|---|---|---|
| Total de códigos | 169 | **176** | +7 |
| Sequência | `P-01`…`P-169` | `P-01`…`P-176` | sem lacunas |
| Códigos duplicados | 0 | **0** | 0 |
| Fusões | 0 | **0** | 0 |
| Renumerações | 0 | **0** | 0 |
| Reclassificações | 0 | **0** | 0 |
| Células alteradas fora das linhas novas | 2 (§33) | **0** | — |
| Linhas da natureza **DEPENDÊNCIAS E BUILD** | 7 | **8** | +1 |
| Linhas da natureza **ASSETS** | 6 | **7** | +1 |
| Linhas da natureza **UI E RESPONSIVIDADE** | 16 | **17** | +1 |
| Linhas da natureza **JOGOS** | 16 | **17** | +1 |
| Linhas da natureza **PRIVACIDADE** | 2 | **3** | +1 |
| Linhas da natureza **OUTRO** | 1 | **3** | +2 |

### 34.7 O que esta seção **não** fez

Não corrigiu nenhum risco. Não fechou, não rebaixou e não reclassificou nenhum código. Não alterou
nenhuma célula fora das sete linhas novas. Não alterou a semântica, a severidade, o *owner* ou a
redação de nenhuma linha da `v6` §9. Não criou ledger concorrente. Não converteu `E-02` a `E-05`.
Não alterou as tabelas históricas das §§3, 12, 13, 15, 16 e 17, que retratam estados anteriores.
Não tocou em *assets*, manifestos, *packs*, código executável nem configuração de *build*. Não fez
*push* e não fez *merge*.

---

*Fim do artefato 9 de 11. **Matriz única e definitiva com 176 riscos**, `P-01` a `P-176`, sem
